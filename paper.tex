...
%that requires the user to send a photo of their environment to a Mechanical Turk worker who sends the position of the requested object within the picture back to the user. 
...
%The error, $e$, is defined as the difference between the desired object and the object currently within view. 
...
% that can be used by a PwVI by adding .% onto a mobile phone that can generate and provide guidance instructions for a user.  
...
%This interface enables ActiVis to describe the location of a waypoint to a user in the pan and tilt dimensions.
...
%\begin{figure}
  %\centering
  %\includegraphics[width=0.4\textwidth]{figures/control_loop_no_controller.png}
  %\caption{The control loop with the human acting as both manipulator and controller. }\label{fig:sys-diagram-no-controller}
%\end{figure}

%Each participant was verbally informed prior to each experiment what the target object was.
...
%and was terminated when the target object was found or the 45 second time limit was reached.
%This process performed once for each object in the experiment set. 
