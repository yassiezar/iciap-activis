...
To describe the waypoints pan and tilt positions, we implemented an improved version of the interface described in~\cite{belloto2013} that exploits a uses a spatialised audio signal.
...
Instead we exploit a human's natural association of high and low sound sources with a high and low sound frequency respectively\todo{cite} and adjust the sound source's pitch as a function of the waypoint's tilt angle. 
...
Each grid square represents a \SI{35}{\degree} angular translation, encompassing a \SI{210}{\degree}$times$\SI{210}{\degree} field of view. 
